...
function downloadCatalog(rows:CatalogProduct[],format:"json"|"csv") {
  const body=format==="json"
    ...
    : [
        ["sku","source","brand","category","gender","shape","size","color","material","frame_type","price","rating","polarized","styles","use_cases"],
        ...rows.map(product=>[product.sku,product.sourceKind,product.brand,product.category,product.gender,product.shape,product.size,product.color,product.material,product.frameType,product.price,product.rating,product.polarized,product.styles.join("|"),product.useCases.join("|")]),
...
function EvaluationView({baseline,hybrid}:{baseline:EvalSummary;hybrid:EvalSummary}) {
  ...
  return <div className="evaluation-view">
    ...
    <section className="catalog-access"><div className="catalog-access-head"><div className="evaluation-heading"><p>CATALOG EXPLORER</p><h2>Inspect and take the synthetic data with you</h2><span>The table previews the exact records used by search. Synthetic rows are labelled and can be downloaded independently from the captured public samples.</span></div><div className="catalog-actions"><button onClick={()=>downloadCatalog(syntheticCatalog,"json")}>Download 576 synthetic rows · JSON</button><button onClick={()=>downloadCatalog(catalog,"csv")}>Download all 600 rows · CSV</button></div></div><div className="catalog-filter"><label>Show <select value={sourceFilter} onChange={event=>setSourceFilter(event.target.value as typeof sourceFilter)}><option value="all">All 600 products</option><option value="synthetic">Synthetic variants only</option><option value="public-sample">Captured public samples only</option></select></label><span>Showing the first {Math.min(12,filteredCatalog.length)} of {filteredCatalog.length}</span></div><div className="catalog-table-wrap"><table><thead><tr><th>Source</th><th>SKU</th><th>Brand</th><th>Vertical</th><th>Shape</th><th>Size</th><th>Material</th><th>Price</th></tr></thead><tbody>{filteredCatalog.slice(0,12).map(product=><tr key={product.id}><td><span className={`catalog-source ${product.sourceKind}`}>{product.sourceKind==="synthetic"?"Synthetic":"Public sample"}</span></td><td>{product.sku}</td><td>{product.brand}</td><td>{product.category}</td><td>{product.shape}</td><td>{product.size}</td><td>{product.material}</td><td>{money(product.price)}</td></tr>)}</tbody></table></div></section>
